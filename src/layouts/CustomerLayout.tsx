...
const tab =
  'flex flex-1 flex-col items-center justify-center gap-0.5 py-2 text-[10px] font-medium text-[var(--color-tr-muted)]'
...
function TabLink({ to, label, icon }: { to: string; label: string; icon: string }) {
  return (
    <NavLink
      to={to}
      className={({ isActive }) =>
        `${tab} ${isActive ? '!text-[var(--color-tr-accent)]' : ''} min-h-[52px]`
      }
    >
      ...
      {showNav ? (
        <nav className="flex shrink-0 border-t border-[var(--color-tr-border)] bg-[var(--color-tr-surface)] pb-[env(safe-area-inset-bottom)]">
          ...
          >
            ...
            <span>장바구니</span>
            {count > 0 ? (
              <span className="absolute right-[calc(50%-22px)] top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-[var(--color-tr-accent)] px-1 text-[10px] font-bold text-[#04120c]">
                {count > 99 ? '99+' : count}
              </span>
            ) : null}
          </NavLink>
          <TabLink to="/admin/store" label="가게" icon="⚙" />
        </nav>
      ) : null}
    </div>
  )
